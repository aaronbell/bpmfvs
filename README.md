...
本規格內相關程式碼以Apache 2.0授權發行。
標音用的注音字型（GenyoMinBopomofo-Regular.ttf／f_bpmfgen.json）則為自由使用，以本專案衍生的注音字型可以使用其它授權條款，亦可作商業發行。但必須明確保留原作者的著作權資訊（NOTICE.txt檔案部份）一起發行。

隨本規格發佈的注音字型，目前已一律統一為OTF 1.1授權，詳細授權內容請參閱隨字型檔附帶的說明文件。
